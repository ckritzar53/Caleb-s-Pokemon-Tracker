...
const createDefaultTeamMember = (species) => {
    ...
    return {
        ...
        stats: { hp: 0, atk: 0, def: 0, spatk: 0, spdef: 0, speed: 0 }, // These will be calculated stats
...
function handleLevelChange(teamIndex) {
    ...
    calculateAndSetMemberStats(member);
}

// --- NEW: Stat Calculation Functions ---
function calculateStat(base, iv, ev, level, natureModifier = 1.0, isHP = false) {
    let stat = 0;
    if (isHP) {
...
function calculateAndSetMemberStats(member) {
    const speciesData = DATA.pokedex.find(p => p.name === member.species);
    if (!speciesData || !speciesData.baseStats) {
        console.warn(`Base stats not found for ${member.species}. Cannot calculate stats.`);
        return;
    }

    const nature = NATURES.find(n => n.name === member.nature) || NATURES.find(n => n.name === 'Hardy'); // Default to Hardy if not found

    const statsToCalculate = ['hp', 'atk', 'def', 'spatk', 'spdef', 'speed'];
    statsToCalculate.forEach(statName => {
        const base = speciesData.baseStats[statName] || 0;
        const iv = parseInt(member.ivs[statName], 10) || 0;
        const ev = parseInt(member.evs[statName], 10) || 0;
        const level = parseInt(member.level, 10) || 1;

        let natureModifier = 1.0;
        if (nature && nature.increased === statName) {
            natureModifier = 1.1;
        } else if (nature && nature.decreased === statName) {
            natureModifier = 0.9;
        }

        member.stats[statName] = calculateStat(base, iv, ev, level, natureModifier, statName === 'hp');
    });
}
...
function openEditorModal(teamIndex) {
    ...
    // Ensure stats are calculated before rendering the editor
    if (appState.team[currentEditingIndex]) {
        calculateAndSetMemberStats(appState.team[currentEditingIndex]);
    }
    renderPokemonEditor();
...
function savePokemonDetails() {
    ...
    ['hp','atk','def','spatk','spdef','speed'].forEach(stat => {
        // We are now saving the input values for EVs and IVs, not the calculated stats
        member.evs[stat] = parseInt(document.getElementById(`edit-ev-${stat}`).value, 10) || 0;
        member.ivs[stat] = parseInt(document.getElementById(`edit-iv-${stat}`).value, 10) || 0;
    });
    ...
    // Recalculate stats after saving details
    calculateAndSetMemberStats(member);
    handleLevelChange(currentEditingIndex); // This also calls calculateAndSetMemberStats, but it's good to have it here too for direct saves
    saveState();
    renderTeamBuilder();
    closeEditorModal();
}
...
function renderPokemonEditor() {
    ...
    // --- NEW: Nature Options ---
    const natureOptions = NATURES.map(n => `<option value="${n.name}" ${member.nature === n.name ? 'selected' : ''}>${n.name}</option>`).join('');
    ...
    document.getElementById('summary-tab').innerHTML = `<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div><label class="form-label">Nature</label><select id="edit-nature" class="form-input">${natureOptions}</select></div>
    ...
    const statsInputs = ['hp','atk','def','spatk','spdef','speed'].map(stat => `
        ...
        <input type="number" id="edit-stat-${stat}" class="form-input form-input-sm" value="${member.stats[stat]}" placeholder="Base" disabled>
        <input type="number" id="edit-ev-${stat}" class="form-input form-input-sm" value="${member.evs[stat]}" placeholder="EVs" min="0" max="252">
        <input type="number" id="edit-iv-${stat}" class="form-input form-input-sm" value="${member.ivs[stat]}" placeholder="IVs" min="0" max="31">
    `).join('');
    ...
    document.getElementById('stats-tab').innerHTML = `<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            ...
            <div class="grid grid-cols-4 gap-2 text-sm items-center text-secondary">
                <span></span><span class="font-medium">Calculated</span><span class="font-medium">EVs</span><span class="font-medium">IVs</span>
                ${statsInputs}
            </div>
        </div>
        <div>
    ...
    document.getElementById('edit-nature').value = member.nature; // Set selected nature
}
...
function setupEventListeners() {
    ...
    document.body.addEventListener('change', e => {
        ...
        // --- NEW: Recalculate stats on change for relevant inputs ---
        if (currentEditingIndex !== null) {
            const member = appState.team[currentEditingIndex];
            if (member) {
                const targetId = e.target.id;
                if (targetId.startsWith('edit-ev-') || targetId.startsWith('edit-iv-') || targetId === 'edit-level' || targetId === 'edit-nature') {
                    // Update member data first
                    if (targetId === 'edit-level') member.level = parseInt(e.target.value, 10) || 1;
                    if (targetId === 'edit-nature') member.nature = e.target.value;
                    if (targetId.startsWith('edit-ev-')) member.evs[targetId.replace('edit-ev-', '')] = parseInt(e.target.value, 10) || 0;
                    if (targetId.startsWith('edit-iv-')) member.ivs[targetId.replace('edit-iv-', '')] = parseInt(e.target.value, 10) || 0;

                    calculateAndSetMemberStats(member);
                    // Update the displayed calculated stats
                    ['hp','atk','def','spatk','spdef','speed'].forEach(stat => {
    ...
    document.body.addEventListener('input', e => {
        ...
        // --- NEW: Real-time stat recalculation for number inputs ---
        if (currentEditingIndex !== null) {
            const member = appState.team[currentEditingIndex];
            if (member) {
                if (targetId.startsWith('edit-ev-') || targetId.startsWith('edit-iv-') || targetId === 'edit-level') {
                    // Update member data first
                    if (targetId === 'edit-level') member.level = parseInt(e.target.value, 10) || 1;
                    if (targetId.startsWith('edit-ev-')) member.evs[targetId.replace('edit-ev-', '')] = parseInt(e.target.value, 10) || 0;
                    if (targetId.startsWith('edit-iv-')) member.ivs[targetId.replace('edit-iv-', '')] = parseInt(e.target.value, 10) || 0;
