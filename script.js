...
function loadState() {
    try {
        ...
        const initialState = { pokedex: {}, team: Array(6).fill(null), gyms: [], tms: [], items: [], sandwiches: [], timeline: [] };
        ...
        if (!appState.timeline || !Array.isArray(appState.timeline)) appState.timeline = []; // Ensure timeline exists
        ...
        appState = { pokedex: {}, team: Array(6).fill(null), gyms: [], tms: [], items: [], sandwiches: [], timeline: [] };
    }
}
...
// --- JOURNAL & TIME LOGIC ---
function logTimelineEvent(message, type = 'auto') {
    const newEvent = {
        id: Date.now(),
    ...
    appState.timeline.unshift(newEvent);
    saveState();
}

function getGameTimeOfDay() {
    const currentHour = new Date().getHours();
    return (currentHour >= 6 && currentHour < 20) ? 'day' : 'night';
}
...
function updatePokedexStatus(pokemonName, clickedStatus) {
    ...
    if (newStatus === 'caught' && currentStatus !== 'caught') {
        logTimelineEvent(`Caught ${pokemonName} for the first time!`);
    }
...
function updateCheckboxState(category, itemName, isChecked) {
    ...
    if (isChecked) {
        items.add(itemName);
        if (category === 'gyms') {
             logTimelineEvent(`Completed the ${itemName}!`);
        }
    } else {
...
function addPokemonToTeam(species) {
    ...
    if (emptySlotIndex !== -1) {
        ...
        logTimelineEvent(`${species} was added to the team.`);
        saveState();
        renderTeamBuilder();
    } else {
...
function removePokemonFromTeam() {
    ...
    const removedMon = appState.team[currentEditingIndex];
    if (removedMon) {
        logTimelineEvent(`${removedMon.nickname} (${removedMon.species}) was removed from the team.`);
    }
...
function handleLevelChange(teamIndex) {
    ...
    if (speciesData?.evolutions.length > 0) {
        ...
        if (member.level >= evolution.at) {
            ...
            logTimelineEvent(`${oldSpecies} evolved into ${evolution.to}!`);
        }
    }
}
...
function renderDashboard() {
    ...
    // New Suggestion Box HTML
    container.innerHTML = `<div id="dashboard-suggestions" class="mb-6"></div> <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">${cards}</div>`;
    renderTimeBasedSuggestions(); // Call the new function
}

function renderTimeBasedSuggestions() {
    const container = document.getElementById('dashboard-suggestions');
    if (!container) return;

    const timeOfDay = getGameTimeOfDay();
    const availablePokemon = DATA.pokedex.filter(p => 
        p.availability?.time?.includes(timeOfDay)
    );

    const suggestions = availablePokemon.filter(p => !appState.pokedex[p.name] || appState.pokedex[p.name] !== 'caught');

    let html = `<div class="card">
        <h3 class="font-semibold text-lg mb-3 text-heading"><i class="fas fa-clock mr-2"></i>Available Now (${timeOfDay})</h3>`;
    
    if (suggestions.length === 0) {
        html += `<p class="text-secondary">No special time-based Pokémon to catch right now.</p>`;
    } else {
        html += `<div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">`
        html += suggestions.slice(0, 10).map(p => {
            const typesHtml = p.types.map(type => `<span class="type-badge ${TYPE_COLORS[type]}">${type}</span>`).join(' ');
            return `<div class="pokedex-item text-center">
                <p class="font-medium text-primary">${p.name}</p>
                <div class="mt-1 flex gap-1 justify-center">${typesHtml}</div>
            </div>`;
        }).join('');
        html += `</div>`;
    }
...
function renderJournal() {
    ...
    const entriesHtml = appState.timeline.map(event => {
        const date = new Date(event.date);
        const formattedDate = `${date.toLocaleDateString()} ${date.toLocaleTimeString()}`;
        const icon = event.type === 'manual' ? 'fa-pen-alt' : 'fa-bolt';
        return `<div class="journal-entry">
            <div class="journal-icon"><i class="fas ${icon}"></i></div>
            <div>
                <p class="text-primary">${event.message}</p>
    ...
    container.innerHTML = `<div class="card">
        <h2 class="text-3xl font-bold text-heading mb-4">My Adventure Journal</h2>
        <div class="mb-6">
            <textarea id="journal-input" class="form-input" rows="3" placeholder="What happened on your journey today?"></textarea>
            <button id="journal-save-btn" class="save-button mt-2">Add Entry</button>
        </div>
...
function setupEventListeners() {
    ...
    document.body.addEventListener('click', e => {
        ...
        if (e.target.id === 'journal-save-btn') {
            const input = document.getElementById('journal-input');
            if (input.value.trim()) {
                logTimelineEvent(input.value.trim(), 'manual');
                input.value = '';
                renderJournal();
            }
        }
